...
import { Upload, Loader2, Download, AlertTriangle, CheckCircle2, RefreshCcw, Smartphone, Settings, Info, ArrowRight, ShieldAlert } from "lucide-react";
    ...
    return (<SidebarProvider>
      <div className="client-enterprise flex min-h-screen w-full">
        ...
        <SidebarInset className="min-w-0 flex-1">
            ...
            <div className="osint-label text-primary/80">{t("pp.title")}</div>
          </header>

          <main className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
              <h1 className="rainbow-text font-display text-3xl font-bold tracking-tight">{t("pp.header")}</h1>
              <p className="mt-2 text-muted-foreground">{t("pp.desc")}</p>
              {!hasAccess && (<div className="mt-4 flex items-center gap-2 rounded border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-200/90">
            ...
            <div className={`grid gap-6 md:grid-cols-2 ${!hasAccess ? 'pointer-events-none opacity-50 grayscale' : ''}`}>
              ...
              <div className="osint-panel p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Input placeholder="Ex: Banco Shadow" value={appName} onChange={(e) => setAppName(e.target.value)} className="rounded-none border-border/60 bg-background/50"/>
                  </div>

                  <div>
                    <label className="mb-2 block font-mono text-[10px] uppercase text-muted-foreground">Arquivo APK Original</label>
                    <div className="relative">
                      <input type="file" accept=".apk" onChange={(e) => setSelectedFile(e.target.files?.[0] || null)} className="hidden" id="apk-upload"/>
                      <label htmlFor="apk-upload" className="flex cursor-pointer items-center justify-between border border-dashed border-border/60 bg-background/50 p-3 transition-colors hover:bg-background/80">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input type="file" accept="image/*" onChange={(e) => setSelectedIcon(e.target.files?.[0] || null)} className="hidden" id="icon-upload"/>
                      <label htmlFor="icon-upload" className="flex cursor-pointer items-center justify-between border border-dashed border-border/60 bg-background/50 p-3 transition-colors hover:bg-background/80">
                        <span className="truncate text-sm text-muted-foreground">
                  ...
                  <Button onClick={handleBuild} disabled={uploading || !selectedFile || !appName} className="w-full rounded-none font-mono uppercase tracking-widest">
              ...
              <div className="osint-panel flex flex-col p-6">
                ...
                <div className="flex-1 space-y-3 overflow-y-auto pr-1">
                  {jobs && jobs.length > 0 ? (jobs.map((job) => (<div key={job.id} className="osint-corners border border-border/40 bg-background/40 p-3">
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-display text-sm font-bold text-foreground">{job.app_name}</span>
                          <span className={`font-mono text-[9px] uppercase tracking-tighter ${job.status === 'completed' ? 'text-neon' :
                job.status === 'failed' ? 'text-danger' : 'text-amber-400'}`}>
                            {job.status}
                          </span>
                        </div>
                        
                        {(job.status === 'pending' || job.status === 'processing') && (<div className="space-y-1.5">
                        ...
                        {job.status === 'completed' && (<div className="flex items-center justify-between gap-2 mt-2">
                            ...
                            <Button variant="ghost" size="sm" className="h-7 rounded-none px-2 font-mono text-[9px] hover:text-neon" onClick={() => job.output_apk_url && window.open(job.output_apk_url, '_blank')}>
                              <Download className="mr-1 h-3 w-3"/> Download
                            </Button>
                          </div>)}

                        {job.status === 'failed' && (<div className="flex items-center gap-1.5 text-danger/80 mt-1">
                      ...
                      </div>))) : (<div className="flex h-40 flex-col items-center justify-center text-center">
