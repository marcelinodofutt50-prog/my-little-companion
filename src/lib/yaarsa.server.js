...
export function panelFromPlanSlug(slug) {
    if (!slug)
        return "v457";
    const s = slug.toLowerCase();
    if (s.includes("lifetime"))
        return "v46";
 ...
 */
export async function resolvePanelFromPlanSlug(slug) {
    await refreshPanelOverrides();
    return panelFromPlanSlug(slug);
}
export async function resolvePanelFromTier(tier) {
    await refreshPanelOverrides();
...
const runtimeOverrides = { v455: {}, v457: {}, v46: {} };
...
export async function withPanelConfig(panel, cfg, fn) {
...
export async function refreshPanelOverrides(force = false) {
    try {
        ...
        console.warn("[yaarsa] overrides indisponíveis, usando ambiente:", e?.message);
    }
}
/** Endereço efetivo do painel (formulário > banco > ambiente > padrão). */
export function panelBaseUrl(panel) {
 ...
 */
export function panelServerHost(panel) {
...
export async function resolvePanelServerHost(panel) {
    await refreshPanelOverrides();
    return panelServerHost(panel);
}
/** Origem da configuração atual do painel (para diagnóstico no admin). */
export function panelConfigSource(panel) {
    if (pinnedOverrides[panel]?.baseUrl)
        return "formulario";
    if (runtimeOverrides[panel].baseUrl)
        return "painel";
    if (process.env[PANEL_CONFIG[panel].baseEnv])
        return "ambiente";
    return "padrao";
}
...
export function yaarsaEndpointsFor(rawBase) {
...
function yaarsaEndpoints(panel) {
    return yaarsaEndpointsFor(panelBaseUrl(panel));
}
export function sanitizeAdminKey(raw, label = "admin key") {
    const cleaned = raw
        .replace(/^\uFEFF/, "")
        .replace(/[\u200B-\u200D\uFEFF]/g, "")
        .replace(/\s+/g, "")
        .replace(/^["']|["']$/g, "");
    if (!cleaned)
        throw new Error(`${label} vazia`);
    if (/[^\x21-\x7E]/.test(cleaned)) {
        throw new Error(`${label} contém caracteres inválidos (use apenas ASCII imprimível)`);
    }
    return cleaned;
}
const PANEL_LABEL = { v455: "4.5.5", v457: "4.5.7", v46: "4.6" };
function yaarsaAdminKey(panel) {
    const cfg = PANEL_CONFIG[panel];
    const override = effective(panel).adminKey;
    const raw = override || process.env[cfg.keyEnv];
    if (!raw) {
        throw new Error(`Nenhuma admin key configurada para o painel ${PANEL_LABEL[panel]}. Preencha o endereço e a admin key no formulário abaixo e clique em "Verificação completa" ou "Salvar e usar".`);
    }
...
function encKey() {
    ...
    if (!raw)
        throw new Error("LICENSE_ENC_KEY not set");
...
export function deriveCredentials(seed) {
    const h = createHash("sha256").update(seed).digest();
    const lower = "abcdefghijkmnpqrstuvwxyz";
    const upper = "ABCDEFGHJKLMNPQRSTUVWXYZ";
    const digits = "23456789";
    const specials = "@#$%!*";
    const pick = (alpha, i) => alpha[h[i] % alpha.length];
...
export function planToSubtype(planSlug) {
    if (planSlug === "login-7d")
        return "7 Days";
    if (planSlug === "login-30d")
        return "1 Month";
    if (planSlug === "login-lifetime")
        return "12 Month";
    return "1 Month";
}
export function expireDateFor(planSlug) {
    ...
    else if (planSlug === "login-30d")
        d.setDate(d.getDate() + 31);
    else if (planSlug === "login-lifetime")
        d.setFullYear(d.getFullYear() + 20);
    else if (planSlug === "trial")
        d.setDate(d.getDate() + 2);
    else
        d.setDate(d.getDate() + 31);
    return d.toISOString().slice(0, 10);
}
function friendlyYaarsaFail(message) {
...
export async function yaarsaRemoveAccount(email, panel = "v457") {
    await refreshPanelOverrides();
    return yaarsaPost({ action: "remove", email, adminkey: yaarsaAdminKey(panel) }, panel);
}
export async function yaarsaExtend(email, newExpireDate, panel = "v457") {
    await refreshPanelOverrides();
...
export async function yaarsaSetPassword(email, password, panel = "v457", username) {
    await refreshPanelOverrides();
    const candidates = ["update", "cpassword", "cpass", "changepassword"];
    let last = { Fail: "Painel não aceitou nenhuma ação de troca de senha" };
...
export async function yaarsaLookupEmail(email, panel) {
    await refreshPanelOverrides();
    const r = await yaarsaPost({ action: "cexpire", email, expire_date: "invalid-probe", adminkey: yaarsaAdminKey(panel) }, panel);
    if (r.Success)
        return { found: true, panel, raw: r };
    const fail = String(r.Fail || "");
    if (NOT_FOUND_RE.test(fail))
        return { found: false, panel, raw: r };
...
export async function yaarsaLookupEmailAllPanels(email) {
    ...
    for (const p of ALL_PANELS) {
        ...
        try {
            ...
            details.push({ panel: p, found: false, error: String(e?.message || e) });
        }
    }
...
const sessionCookies = { v455: "", v457: "", v46: "" };
const warmedUp = { v455: false, v457: false, v46: false };
function browserHeaders(url, panel, extra = {}) {
    const h = {
    ...
    if (sessionCookies[panel])
        h.Cookie = sessionCookies[panel];
    return h;
}
function captureCookies(res, panel) {
    const anyHeaders = res.headers;
    const list = typeof anyHeaders.getSetCookie === "function"
    ...
    if (parts.length)
        sessionCookies[panel] = parts.join("; ");
}
async function warmup(url, panel) {
    if (warmedUp[panel])
        return;
    try {
...
async function yaarsaPost(fields, panel) {
    const payload = { ...fields };
    delete payload.admin_key;
    const body = JSON.stringify(payload);
    const maskKey = (k) => k.length <= 4
        ? "*".repeat(k.length)
    ...
    if (debugPayload.password)
        debugPayload.password = `***(len=${payload.password.length})`;
    ...
    const endpoints = Array.from(new Set(proxyUrl && panel === "v457" ? [proxyUrl, ...directEndpoints] : directEndpoints));
    const map = {
        "1001": "ação inválida",
    ...
    let lastFail = { Fail: "Nenhum painel respondeu à requisição" };
    let lastNetworkErr = null;
    const kindOf = (u) => proxyUrl && u === proxyUrl ? "PROXY" : "DIRECT";
    ...
    for (const url of endpoints) {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
            ...
            try {
                ...
                const originHint = headerSnapshot["cf-ray"] || headerSnapshot["cf-cache-status"]
                    ? "cloudflare"
            ...
            try {
                ...
                if (parsed.Fail) {
                    ...
                    const isLookupMiss = action === "cexpire" &&
                    ...
                    await persistLog({
                        ...
                        payload: debugPayload,
                        response_body: text,
                        error: isLookupMiss ? null : friendly,
                        context: { routing: routingSummary, response: responseMeta },
                    });
