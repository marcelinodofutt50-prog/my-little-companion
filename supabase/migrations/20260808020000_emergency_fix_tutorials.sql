...
-- Ensuring tutorials table and bucket exist with correct permissions

-- 1. Create tutorials table if not exists (redundant but safe)
...
-- 3. Grant permissions (CRITICAL for PostgREST cache errors)
...
-- Note: bucket creation is usually handled via supabase--storage_create_bucket, but we ensure policies here
DO $$
BEGIN
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Staff Uploads' AND tablename = 'objects') THEN
